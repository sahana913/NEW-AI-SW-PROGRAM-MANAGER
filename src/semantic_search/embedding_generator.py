    ...
    try:
        ...
        for i, chunk_text in enumerate(chunks):
            ...
            chunk_embeddings.append(
                {"chunk_index": i, "text": chunk_text, "embedding": embedding}
            )
...
def store_embeddings_in_opensearch(
    ...
    try:
        ...
        for chunk_data in chunk_embeddings:
            ...
            opensearch_client.index(index=index_name, id=doc_id, body=document_chunk)

        logger.info(
            f"Stored {len(chunk_embeddings)} chunks in OpenSearch index: {index_name}"
        )
