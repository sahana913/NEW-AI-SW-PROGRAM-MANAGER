...
def store_pdf_in_s3(tenant_id: str, report_id: str, pdf_bytes: bytes) -> str:
    """
    ...
    try:
        ...
        raise DataError(f"Failed to store PDF in S3: {str(e)}", data_source="S3")


def generate_pdf_download_url(s3_key: str, expiration: int = 86400) -> str:
    """
...
def get_pdf_from_s3(tenant_id: str, report_id: str) -> bytes:
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchKey":
            ...
            raise DataError(f"PDF not found for report {report_id}", data_source="S3")
        else:
            logger.error(
            ...
            raise DataError(
                f"Failed to retrieve PDF from S3: {str(e)}", data_source="S3"
            )
