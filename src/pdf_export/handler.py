...
from .pdf_storage import generate_pdf_download_url, store_pdf_in_s3
from .pdf_generator import convert_html_to_pdf, validate_pdf_generation
from shared.logger import get_logger
from shared.errors import DataError, ProcessingError, ValidationError
...
def get_html_from_s3(tenant_id: str, report_id: str) -> str:
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchKey":
            ...
            raise DataError(
                f"HTML report not found for report {report_id}", data_source="S3"
            )
        else:
            logger.error(
            ...
            raise DataError(
                f"Failed to retrieve HTML report: {str(e)}", data_source="S3"
            )


def send_failure_notification(
    tenant_id: str, user_id: str, report_id: str, error_message: str
):
    """
    ...
    try:
        ...
        sns.publish(
            ...
            MessageAttributes={
                ...
                "notification_type": {
                    "DataType": "String",
                    "StringValue": "PDF_GENERATION_FAILURE",
                },
            },
        )
...
def batch_export_reports_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    for report_id in report_ids:
        try:
            ...
            results["failed"].append({"reportId": report_id, "error": str(e)})
