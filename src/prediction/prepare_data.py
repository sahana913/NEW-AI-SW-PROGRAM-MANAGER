...
def main():
    ...
    parser = argparse.ArgumentParser(description='Prepare training data for delay prediction model')
    parser.add_argument('--tenant-id', type=str, help='Optional tenant ID to filter data')
    ...
    try:
        ...
        for split_name, df in data_splits.items():
            if not df.empty:
                ...
                logger.info(f"Saved {split_name} data to {output_file} ({len(df)} records)")
            else:
        ...
        metadata = {
            ...
            'features': list(data_splits['train'].columns) if not data_splits['train'].empty else []
        }
