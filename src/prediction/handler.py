...
class PredictionService:
    ...
    def __init__(self):
        ...
        self._risks_table = None

    @property
    def predictions_table(self):
        if self._predictions_table is None:
            self._predictions_table = get_dynamodb().Table(PREDICTIONS_TABLE)
        return self._predictions_table

    @property
    def risks_table(self):
        if self._risks_table is None:
            self._risks_table = get_dynamodb().Table(RISKS_TABLE)
        return self._risks_table
...
def _generate_workload_risk_alert(
    ...
    risks_table = get_dynamodb().Table(RISKS_TABLE)
