...
class TrainingDataPreparation:
    ...
    def extract_sprint_metrics(self, project_ids: List[str]) -> pd.DataFrame:
        ...
        for col in ['velocity', 'completed_points', 'planned_points', 'completion_rate']:
    ...
    def extract_backlog_metrics(self, project_ids: List[str]) -> pd.DataFrame:
        ...
        if 'avg_age_days' in df.columns:
            df['avg_age_days'] = df['avg_age_days'].apply(lambda x: float(x) if isinstance(x, Decimal) else x)
    ...
    def engineer_features(self, projects_df: pd.DataFrame, sprints_df: pd.DataFrame,
                         backlog_df: pd.DataFrame, milestones_df: pd.DataFrame,
                         dependencies_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        for _, project in projects_df.iterrows():
            ...
            if not project_backlog.empty:
                ...
                avg_age = project_backlog['avg_age_days'].values[0] if project_backlog['avg_age_days'].values[0] else 0
            else:
                total_backlog = 0
                open_backlog = 0
                backlog_ratio = 0
            ...
            # Milestone features
            project_milestones = milestones_df[milestones_df['project_id'] == project_id]
            if not project_milestones.empty:
                total_milestones = project_milestones['total_milestones'].values[0]
                completed_milestones = project_milestones['completed_milestones'].values[0]
                ...
                avg_milestone_completion = project_milestones['avg_completion_percentage'].values[0]
            else:
                total_milestones = 0
            ...
            # Dependency features
            project_dependencies = dependencies_df[dependencies_df['project_id'] == project_id]
            if not project_dependencies.empty:
                total_dependencies = project_dependencies['total_dependencies'].values[0]
                active_dependencies = project_dependencies['active_dependencies'].values[0]
                blocking_dependencies = project_dependencies['blocking_dependencies'].values[0]
            else:
    ...
    def label_delay_outcomes(self, features_df: pd.DataFrame, milestones_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        for _, row in labeled_data.iterrows():
            ...
            if is_delayed:
                ...
                estimated_delay = row['delayed_milestones'] * 14  # Assume 2 weeks per delayed milestone
            else:
                estimated_delay = 0
        ...
        logger.info(f"Labeled {len(labeled_data)} projects: {sum(delay_labels)} delayed, {len(delay_labels) - sum(delay_labels)} on-time")
        
        return labeled_data
    
    def split_train_val_test(self, labeled_data: pd.DataFrame, 
        ...
        # Shuffle data
        shuffled_data = labeled_data.sample(frac=1, random_state=42).reset_index(drop=True)
