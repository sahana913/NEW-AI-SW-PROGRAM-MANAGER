...
class DelayPredictionModelTrainer:
    ...
    def __init__(
        ...
        if role_arn:
            ...
            try:
                ...
                logger.warning(
                    f"Could not get execution role: {e}. Using environment variable."
                )
                self.role = os.environ.get("SAGEMAKER_ROLE_ARN", "")

        logger.info(
            f"Initialized model trainer with bucket: {bucket_name}, region: {region}"
        )
    ...
    def upload_training_data_to_s3(
        ...
        for name, df in [
            ...
            feature_cols = [
                ...
                if col not in ["project_id", "tenant_id", "is_delayed", "delay_days"]
            ]
    ...
    def train_binary_classifier(
        ...
        container = sagemaker.image_uris.retrieve(
            framework="xgboost", region=self.region, version="1.5-1"
        )
    ...
    def evaluate_model(
        ...
        try:
            job_description = sm_client.describe_training_job(
    ...
    def save_model_metadata(
        ...
        # Save metadata to S3
        metadata_key = (
            f"{output_path.replace(f's3://{self.bucket_name}/', '')}/metadata.json"
        )
    ...
    def train_models(
        ...
        classifier = self.train_binary_classifier(
            s3_uris["train"], s3_uris["validation"], f"{output_path}/classifier"
        )

        classifier_metrics = self.evaluate_model(
            classifier, s3_uris.get("test", ""), test_df, "classifier"
        )
        ...
        if regressor:
            regressor_metrics = self.evaluate_model(
                regressor, s3_uris.get("test", ""), test_df, "regressor"
            )

        # Save metadata
        metadata = self.save_model_metadata(
            classifier, regressor, classifier_metrics, regressor_metrics, output_path
        )
