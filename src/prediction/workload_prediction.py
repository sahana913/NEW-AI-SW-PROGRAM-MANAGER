...
class WorkloadPredictionTrainer:
    ...
    def prepare_workload_training_data(self, db_connection) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        logger.info(f"Prepared workload data: train={len(train_df)}, val={len(val_df)}, test={len(test_df)}")
...
"""
    ...
    def train_workload_model(self, train_df: pd.DataFrame, val_df: pd.DataFrame) -> sagemaker.estimator.Estimator:
        """
...
class WorkloadPredictionService:
    ...
    def predict_workload_imbalance(self, project_id: str, db_connection) -> Dict[str, Any]:
        """
        ...
        # Calculate features
        from decimal import Decimal
        utilizations = [float(r[3]) if isinstance(r[3], Decimal) else r[3] for r in resources]
        ...
        for resource in resources:
            ...
            utilization = float(resource[3]) if isinstance(resource[3], Decimal) else resource[3]
        ...
        confidence_score = min(0.95, team_size / 10)  # Higher confidence with more team members
    ...
    def _generate_recommendations(self, overallocated: List[Dict], 
                                 underallocated: List[Dict],
                                 imbalance_score: float) -> List[str]:
        """Generate workload rebalancing recommendations"""
        recommendations = []
        
        if imbalance_score > 40:
            recommendations.append("High workload imbalance detected - immediate rebalancing recommended")
