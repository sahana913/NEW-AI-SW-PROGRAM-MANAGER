...
def main():
    ...
    parser.add_argument(
        "--bucket", type=str, required=True, help="S3 bucket for model artifacts"
    )
    ...
    try:
        ...
        logger.info(
            f"Loaded training data: train={len(train_df)}, val={len(val_df)}, test={len(test_df)}"
        )
    ...
    try:
        trainer = DelayPredictionModelTrainer(
            bucket_name=args.bucket, role_arn=args.role_arn, region=args.region
        )
    except Exception as e:
