...
def detect_cross_tenant_access(
    ...
    """
    violation_id = f"VIOLATION-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}-{user_id[:8]}"
    ...
    violation_details = {
        ...
        'request_context': {
            ...
            'source_ip': request_context.get('requestContext', {}).get('identity', {}).get('sourceIp'),
            'user_agent': request_context.get('requestContext', {}).get('identity', {}).get('userAgent')
        }
    }
