...
def calculate_velocity_trend(sprints: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    velocities = [
        s.get("velocity", 0) for s in sprints if s.get("velocity") is not None
    ]
    ...
    if moving_average > 0:
        decline_percentage = (
            (moving_average - current_velocity) / moving_average
        ) * 100
...
def detect_velocity_decline(
    sprints: List[Dict[str, Any]], velocity_metrics: Dict[str, Any]
    ...
    decline_percentage = (
        (previous_velocity - current_velocity) / previous_velocity
    ) * 100
...
def analyze_velocity_risk(project_id: str, tenant_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(
            ...
            extra={"project_id": project_id, "metrics": velocity_metrics},
        )
