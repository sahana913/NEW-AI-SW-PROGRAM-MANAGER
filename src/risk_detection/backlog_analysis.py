...
def query_backlog_metrics(project_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "average_age": (
                float(result.get("average_age", 0)) if result.get("average_age") else 0
            ),
        }
...
def calculate_backlog_growth_rate(historical_data: List[Dict[str, Any]]) -> float:
    """
...
def detect_backlog_growth_risk(
    backlog_metrics: Dict[str, Any], growth_rate: float, completion_rate: float
    ...
    if rapid_growth or excessive_backlog:
        # Determine severity
        if growth_rate >= 50 or (
            completion_rate > 0 and open_items > 3 * completion_rate
        ):
            severity = "CRITICAL"
        elif growth_rate >= 40 or (
            completion_rate > 0 and open_items > 2.5 * completion_rate
        ):
            severity = "HIGH"
...
def analyze_backlog_risk(project_id: str, tenant_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if backlog_metrics["total_items"] == 0:
            logger.info(
                f"No backlog items found for project", extra={"project_id": project_id}
            )
            return None
