    ...
    with get_db_connection() as conn:
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if not commit:
                    conn.rollback()  # Clean state when not committing
...
def insert_project(
    ...
    try:
        ...
        raise  # Re-raise DataError directly — do not retry
    except Exception as e:
        # Only retry for ON CONFLICT syntax errors (e.g., missing unique constraint)
        query_simple = """
            INSERT INTO projects (tenant_id, project_name, source, external_project_id, last_sync_at)
            VALUES (%s, %s, %s, %s, NOW())
            RETURNING project_id::text
