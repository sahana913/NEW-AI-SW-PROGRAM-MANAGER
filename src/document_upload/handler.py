...
from shared.logger import log_api_request, log_data_modification, log_error
...
if not DOCUMENTS_BUCKET:
    ...
    warnings.warn("DOCUMENTS_BUCKET environment variable is not set")
...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        validate_required_fields(
            body, ["projectId", "documentType", "fileName", "fileSize", "contentType"]
        )
        ...
        if not DOCUMENTS_BUCKET:
            raise ValidationError("DOCUMENTS_BUCKET is not configured", field="server")
