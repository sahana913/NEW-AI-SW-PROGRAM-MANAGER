...
try:
    from .cache_manager import cache_dashboard_data, get_cached_data, invalidate_cache
...
def get_dashboard_overview_handler(
...
@with_error_handling
@with_tenant_isolation
def get_project_dashboard_handler(
    event: Dict[str, Any], context: Any
) -> Dict[str, Any]:
    """
...
def get_metrics_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if metric_type not in ["velocity", "backlog", "utilization"]:
        raise ValidationError(
            f"Invalid metricType: {metric_type}. Must be velocity, backlog, or utilization"
        )

    if time_range not in ["7d", "30d", "90d", "all"]:
        raise ValidationError(
            f"Invalid timeRange: {time_range}. Must be 7d, 30d, 90d, or all"
        )
    ...
    try:
        ...
        if not metrics_data:
            return {
                ...
                "body": json.dumps({"message": "No metrics data found"}),
            }
