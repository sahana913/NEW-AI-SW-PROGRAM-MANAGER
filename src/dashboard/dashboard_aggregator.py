...
def get_dashboard_overview(tenant_id: str, project_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    """
...
def get_project_summaries(tenant_id: str, project_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for project in projects:
            ...
            if health_data:
                ...
                project['ragStatus'] = determine_rag_status(health_data.get('health_score', 0))
    ...
    try:
        ...
        if results:
            ...
            if milestone.get('completionPercentage') is not None:
                milestone['completionPercentage'] = float(milestone['completionPercentage'])
            return milestone
        return None
    except Exception as e:
...
def get_recent_risks(tenant_id: str, project_ids: Optional[List[str]] = None, limit: int = 10) -> List[Dict[str, Any]]:
    """
...
def get_upcoming_milestones(tenant_id: str, project_ids: Optional[List[str]] = None, days_ahead: int = 14) -> List[Dict[str, Any]]:
    """
    Get upcoming milestones.
...
def get_velocity_trend(project_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        values = [float(r['velocity']) if r.get('velocity') is not None else 0 for r in results]
...
def get_metrics(tenant_id: str, project_id: str, metric_type: str, time_range: str) -> Optional[Dict[str, Any]]:
    """
...
def get_backlog_metrics(project_id: str, start_date: Optional[Any]) -> Optional[Dict[str, Any]]:
    """
    Get backlog metrics.
...
def get_utilization_metrics(project_id: str, start_date: Optional[Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        values = [float(r['avg_utilization']) if r.get('avg_utilization') is not None else 0 for r in results]
