...
class AzureDevOpsAPIClient:
    ...
    def fetch_work_items(self, project: str, wiql_query: str = None) -> List[Dict[str, Any]]:
        """
        ...
        # Get work item IDs
        work_item_ids = [str(item['id']) for item in work_item_refs[:200]]  # Limit to 200
    ...
    def fetch_iteration_work_items(self, project: str, team: str, iteration_id: str) -> Dict[str, Any]:
        """
    ...
    def fetch_releases(self, project: str) -> List[Dict[str, Any]]:
        ...
        release_url = self.organization_url.replace('dev.azure.com', 'vsrm.dev.azure.com')
...
def transform_sprint_data(iteration: Dict[str, Any], work_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    for item in work_items:
        ...
        effort = fields.get('Microsoft.VSTS.Scheduling.Effort', 0) or fields.get('Microsoft.VSTS.Scheduling.StoryPoints', 0)
...
def transform_backlog_data(work_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for item in work_items:
        ...
        priority_name = f"Priority {priority}" if isinstance(priority, int) else str(priority)
...
def transform_dependency_data(work_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in work_items:
        ...
        for relation in relations:
            ...
            if 'Predecessor' in rel_type or 'Successor' in rel_type or 'Dependency' in rel_type:
                url = relation.get('url', '')
                # Extract target work item ID from URL
                if '/workItems/' in url:
                    target_id = url.split('/workItems/')[-1]
                    
                    dependency_type = 'BLOCKS' if 'Predecessor' in rel_type else 'RELATES_TO'
    ...
    try:
        ...
        try:
            ...
            for iteration in iterations[:5]:  # Limit to recent 5 sprints
                try:
                    iteration_work_items_response = azure_client.fetch_iteration_work_items(
                        project_name, project_name, iteration['id']
                    )
                    
                    # Fetch full work item details
                    work_item_refs = iteration_work_items_response.get('workItemRelations', [])
                    if work_item_refs:
                        work_item_ids = [str(ref['target']['id']) for ref in work_item_refs if 'target' in ref]
                        if work_item_ids:
                            ids_param = ','.join(work_item_ids[:100])
                            work_items_url = f"{organization_url}/_apis/wit/workitems?ids={ids_param}&api-version=7.0"
                            iteration_items_response = azure_client._make_request('GET', work_items_url)
                    ...
                    logger.warning(f"Failed to fetch iteration {iteration.get('name')}: {str(e)}")
                    continue
        ...
        try:
            ...
            logger.error(f"Failed to transform dependencies for {project_name}: {str(e)}")
            dependencies = []
