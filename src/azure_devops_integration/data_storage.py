"""Data storage handler for Azure DevOps integration.

Validates and stores fetched data in RDS PostgreSQL.

Validates:
- Property 8: Schema Validation (Requirements 4.5, 4.6)
- Property 9: Metadata Persistence (Requirements 4.7)
"""
...
def store_azure_devops_project_data(db_conn, project_data: Dict[str, Any]) -> None:
    """
    Store Azure DevOps project data in RDS PostgreSQL.
    ...
    try:
        ...
        source = project_data.get("source", "AZURE_DEVOPS")
        last_sync_at = project_data.get("lastSyncAt")

        logger.info(f"Storing Azure DevOps project: {project_name} (ID: {project_id})")
        ...
        logger.info(
            f"Successfully stored Azure DevOps project {project_name}: "
        ...
        db_conn.rollback()
        error_msg = f"Failed to store Azure DevOps project data: {str(e)}"
