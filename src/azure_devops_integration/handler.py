...
@with_audit_logging
@with_tenant_isolation
def configure_azure_devops_integration(
    ...
    try:
        ...
        validate_required_fields(body, ["organizationUrl", "projectName", "pat"])
        ...
        if not isinstance(project_name, str) or len(project_name.strip()) == 0:
            raise ValidationError(
                "projectName must be a non-empty string", field="projectName"
            )

        # Validate PAT
        if not isinstance(pat, str) or len(pat.strip()) == 0:
            raise ValidationError(
                "pat (Personal Access Token) must be a non-empty string", field="pat"
            )
        ...
        secret_name = (
            f"{SECRETS_MANAGER_PREFIX}/azure-devops/{tenant_id}/{integration_id}"
        )
        ...
        logger.error(
            f"Unexpected error in configure_azure_devops_integration: {str(e)}"
        )
        raise


# Import data fetcher — use absolute import compatible with Lambda execution
try:
    from azure_devops_integration.data_fetcher import fetch_azure_devops_data
except ImportError:
