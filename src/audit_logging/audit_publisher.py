...
def publish_authentication_event(
    user_id: str, email: str, success: bool, reason: Optional[str] = None
) -> bool:
    """
    ...
    try:
        ...
        if response["FailedEntryCount"] > 0:
            logger.error(
                ...
                extra={"user_id": user_id, "failed_entries": response["Entries"]},
            )
            return False
        ...
        log_error(
            logger,
            e,
            context={"function": "publish_authentication_event", "user_id": user_id},
        )
        return False
