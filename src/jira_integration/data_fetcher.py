...
class JiraAPIClient:
    ...
    def __init__(self, jira_url: str, auth_type: str, credentials: Dict[str, Any]):
        ...
        if auth_type == "API_TOKEN":
            ...
            self.session.auth = HTTPBasicAuth(
                credentials.get("email", "user@example.com"), credentials["apiToken"]
            )
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1 and e.response.status_code >= 500:
                    ...
                    raise DataError(
                        f"Jira API request failed: {str(e)}", data_source="JiraAPI"
                    )
        ...
        raise DataError(
            "Maximum retries exceeded for Jira API request", data_source="JiraAPI"
        )
    ...
    def fetch_sprints(self, board_id: str) -> List[Dict[str, Any]]:
        ...
        response = self._make_request(
            "GET", endpoint, params={"state": "active,closed"}
        )
    ...
    def fetch_issues_by_jql(
        self, jql: str, max_results: int = 1000
    ) -> List[Dict[str, Any]]:
        """
...
def transform_sprint_data(
    sprint_data: Dict[str, Any], sprint_report: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def transform_resource_data(issues: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for issue in issues:
        ...
        if assignee:
            ...
            time_spent = (
                time_tracking.get("timeSpentSeconds", 0) / 3600
            )  # Convert to hours
...
def transform_dependency_data(
    issue_links: List[Dict[str, Any]], issue_key: str
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        try:
            storage_results = store_multiple_projects(
                tenant_id=tenant_id, projects=project_data_list, source="JIRA"
            )
