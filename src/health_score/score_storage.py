...
def store_health_score_history(
    project_id: str, tenant_id: str, health_score_data: Dict[str, Any]
) -> str:
    """
    ...
    try:
        ...
        raise DataError(
            f"Failed to store health score history: {str(e)}", data_source="Database"
        )
...
def publish_health_score_event(
    ...
    try:
        ...
        if response.get("FailedEntryCount", 0) > 0:
            logger.error(
                f"Failed to publish health score event", extra={"response": response}
            )
        else:
            logger.info(
