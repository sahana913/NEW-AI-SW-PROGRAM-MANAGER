...
from .score_storage import publish_health_score_event, store_health_score_history
...
def data_refresh_trigger_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not project_id or not tenant_id:
            logger.warning(
                f"Missing project_id or tenant_id in event", extra={"event": event}
            )
            return {
                "statusCode": 400,
                "body": json.dumps({"message": "Missing required fields in event"}),
            }
...
@with_logging
@with_error_handling
def scheduled_recalculation_handler(
    ...
    try:
        ...
        if not projects:
            ...
            return {
                "statusCode": 200,
                "body": json.dumps(
                    {"message": "No projects to process", "projects_processed": 0}
                ),
            }

        logger.info(f"Found {len(projects)} projects for health score recalculation")
        ...
        logger.error(
            f"Scheduled health score recalculation failed", extra={"error": str(e)}
        )
        raise
