...
@with_logging
@with_error_handling
def calculate_health_score_handler(
    ...
    try:
        ...
        health_score_data = calculate_health_score(
            project_id=project_id, tenant_id=tenant_id, custom_weights=custom_weights
        )
    ...
    try:
        ...
        if not health_score:
            return {
                ...
                "body": json.dumps({"message": "No health score found for project"}),
            }
...
@with_error_handling
@with_tenant_isolation
def get_health_score_history_handler(
    event: Dict[str, Any], context: Any
) -> Dict[str, Any]:
    """
