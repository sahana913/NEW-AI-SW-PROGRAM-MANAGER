...
EMAIL_DELIVERY_LOGS_TABLE = os.environ.get(
    "EMAIL_DELIVERY_LOGS_TABLE", "ai-sw-pm-email-delivery-logs"
)
EMAIL_PREFERENCES_TABLE = os.environ.get(
    "EMAIL_PREFERENCES_TABLE", "ai-sw-pm-email-preferences"
)
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not filtered_recipients:
        logger.warning(
            "No recipients to send to after filtering", extra={"report_id": report_id}
        )
        return {
            "statusCode": 200,
            "body": json.dumps(
    ...
    while attempt < max_retries:
        ...
        try:
            ...
            message_id = email_sender.send_report_email(
                recipient=recipient, report_details=report_details, tenant_id=tenant_id
            )
