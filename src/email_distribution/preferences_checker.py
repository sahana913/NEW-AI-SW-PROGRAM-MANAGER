...
class PreferencesChecker:
    ...
    def can_send_email(self, recipient: str, tenant_id: str) -> bool:
        ...
        try:
            response = self.table.get_item(
                Key={"PK": f"TENANT#{tenant_id}", "SK": f"EMAIL#{recipient}"}
            )
            ...
            logger.error(
                ...
                extra={"recipient": recipient, "tenant_id": tenant_id, "error": str(e)},
            )
            # On error, default to allowing email (fail open)
            return True

    def set_unsubscribe(
        self, recipient: str, tenant_id: str, unsubscribed: bool = True
    ) -> bool:
        """
    ...
    def get_preference(self, recipient: str, tenant_id: str) -> Optional[dict]:
        """
