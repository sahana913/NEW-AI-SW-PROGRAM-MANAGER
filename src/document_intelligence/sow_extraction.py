...
def extract_milestones_from_sow(
    ...
    try:
        ...
        extraction_ids = _store_milestone_extractions(
            document_id=document_id, tenant_id=tenant_id, milestones=milestones
        )

        logger.info(
            f"Extracted {len(milestones)} milestones from document {document_id}"
        )
        ...
        raise ProcessingError(
            f"Milestone extraction failed: {str(e)}", processing_type="sow_extraction"
        )
    ...
    try:
        ...
        for milestone in milestones:
            ...
            if "milestoneName" not in milestone or "dueDate" not in milestone:
                logger.warning(
                    f"Skipping milestone with missing required fields: {milestone}"
                )
                continue
...
def _store_milestone_extractions(
    document_id: str, tenant_id: str, milestones: List[Dict[str, Any]]
) -> List[str]:
    """
