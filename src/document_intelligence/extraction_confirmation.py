...
from shared.errors import DataError, ValidationError
import json
import os
import sys
    ...
    try:
        ...
        if not extraction:
            raise ValidationError(
                f"Extraction not found: {extraction_id}", field="extractionId"
            )

        # Verify tenant ID matches
        if extraction.get("tenantId") != tenant_id:
            raise ValidationError(
                "Extraction does not belong to this tenant", field="tenantId"
            )
        ...
        if confirmed:
            ...
            content = (
                ...
                else json.loads(extraction.get("content", "{}"))
            )

            if extraction_type == "MILESTONE":
                _create_milestone_record(
                    tenant_id=tenant_id, extraction=extraction, content=content
                )
        ...
        raise DataError(
            f"Extraction confirmation failed: {str(e)}", data_source="DynamoDB"
        )


def get_extractions_for_document(
    document_id: str, tenant_id: str, status_filter: Optional[str] = None
) -> list[Dict[str, Any]]:
    """
    ...
    try:
        ...
        extractions.sort(
            key=lambda x: (not x.get("requiresReview", False), x.get("confidence", 0))
        )
        ...
        raise DataError(
            f"Failed to retrieve extractions: {str(e)}", data_source="DynamoDB"
        )
...
def _get_extraction(document_id: str, extraction_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = table.get_item(
            Key={"PK": f"DOCUMENT#{document_id}", "SK": f"EXTRACTION#{extraction_id}"}
        )
...
def _update_extraction_status(
    ...
    try:
        ...
        update_expression = (
            "SET #status = :status, confirmedBy = :user, confirmedAt = :timestamp"
        )
...
def _create_milestone_record(
    tenant_id: str, extraction: Dict[str, Any], content: Dict[str, Any]
) -> None:
    """
    ...
    try:
        ...
        if not project_id:
            logger.warning(
                f"No project ID found for document {document_id}, skipping milestone creation"
            )
            return
        ...
        insert_milestones(
            project_id=project_id, milestones=[milestone_data], source="SOW_EXTRACTION"
        )

        logger.info(
            f"Created milestone record for extraction {extraction.get('extractionId')}"
        )
...
def _create_sla_monitoring_rule(
    ...
    try:
        ...
        logger.info(
            f"SLA monitoring rule created for extraction {extraction.get('extractionId')}"
        )
        logger.info(
            f"SLA Rule: {content.get('slaMetricName')} - {content.get('targetThreshold')}"
        )
...
def _get_project_id_from_document(document_id: str) -> Optional[str]:
    ...
    try:
        ...
        db = _get_dynamodb_resource()
        documents_table = db.Table(
            os.environ.get("DOCUMENTS_TABLE", "ai-sw-pm-documents")
        )
