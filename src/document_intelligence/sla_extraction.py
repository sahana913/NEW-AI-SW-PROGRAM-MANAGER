...
def extract_sla_clauses_from_contract(
    ...
    try:
        ...
        logger.info(f"Extracted {len(sla_clauses)} SLA clauses from document {document_id}")
        ...
        raise ProcessingError(
            f"SLA extraction failed: {str(e)}",
            processing_type='sla_extraction'
        )
    ...
    try:
        ...
        for clause in sla_clauses:
            ...
            if not all(field in clause for field in required_fields):
                logger.warning(f"Skipping SLA clause with missing required fields: {clause}")
                continue
