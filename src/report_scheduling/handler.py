...
def list_schedules_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "body": json.dumps(
                {"schedules": formatted_schedules, "count": len(formatted_schedules)}
            ),
        }
...
def delete_schedule_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "body": json.dumps(
                {"message": "Schedule deleted successfully", "scheduleId": schedule_id}
            ),
        }
...
def _list_schedules(
    ...
    try:
        ...
        if report_type_filter:
            schedules = [
                s for s in schedules if s.get("reportType") == report_type_filter
            ]
        ...
        logger.error(
            f"Failed to list schedules: {str(e)}", extra={"tenant_id": tenant_id}
        )
        return []


def _update_schedule(
    tenant_id: str, schedule_id: str, updates: Dict[str, Any]
) -> Dict[str, Any]:
    """
