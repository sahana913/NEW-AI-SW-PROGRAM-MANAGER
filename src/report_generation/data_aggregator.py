...
def query_project_health_scores(
    ...
    try:
        ...
        raise DataError(
            f"Failed to query project health scores: {str(e)}", data_source="RDS"
        )
...
def query_completed_milestones(
    ...
    try:
        ...
        for milestone in milestones or []:
            ...
            if "completion_percentage" in milestone and isinstance(
                milestone["completion_percentage"], Decimal
            ):
        ...
        raise DataError(
            f"Failed to query completed milestones: {str(e)}", data_source="RDS"
        )
...
def query_upcoming_milestones(
    ...
    try:
        ...
        raise DataError(
            f"Failed to query upcoming milestones: {str(e)}", data_source="RDS"
        )
...
def query_active_risks(
    ...
    try:
        ...
        raise DataError(
            f"Failed to query active risks: {str(e)}", data_source="DynamoDB"
        )


def query_velocity_trends(
    tenant_id: str, project_ids: Optional[List[str]] = None, num_sprints: int = 8
    ...
    try:
        ...
        raise DataError(f"Failed to query velocity trends: {str(e)}", data_source="RDS")
    ...
    try:
        ...
        for row in results:
            ...
            backlog_by_project[project_id] = {
                ...
                "avg_age_days": (
                    float(row.get("avg_age_days", 0)) if row.get("avg_age_days") else 0
                ),
            }
        ...
        raise DataError(f"Failed to query backlog status: {str(e)}", data_source="RDS")
...
def query_predictions(
    ...
    try:
        ...
        predictions_table = dynamodb.Table(
            os.environ.get("PREDICTIONS_TABLE", "ai-sw-pm-predictions")
        )
        ...
        raise DataError(
            f"Failed to query predictions: {str(e)}", data_source="DynamoDB"
        )
