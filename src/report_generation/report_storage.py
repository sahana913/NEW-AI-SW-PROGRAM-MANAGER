...
def store_report_html(tenant_id: str, report_id: str, html_content: str) -> str:
    """
    ...
    try:
        ...
        raise DataError(f"Failed to store report in S3: {str(e)}", data_source="S3")
...
def generate_presigned_url(s3_key: str, expiration: int = 86400) -> str:
    ...
    try:
        ...
        raise DataError(
            f"Failed to generate pre-signed URL: {str(e)}", data_source="S3"
        )
...
def store_report_metadata(
    ...
    try:
        ...
        raise DataError(
            f"Failed to store report metadata: {str(e)}", data_source="DynamoDB"
        )
...
def get_report_metadata(tenant_id: str, report_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = reports_table.get_item(
            Key={"PK": f"TENANT#{tenant_id}", "SK": f"REPORT#{report_id}"}
        )
        ...
        raise DataError(
            f"Failed to retrieve report metadata: {str(e)}", data_source="DynamoDB"
        )


def list_reports(
    tenant_id: str, report_type: Optional[str] = None, limit: int = 50
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if report_type:
            ...
            response = reports_table.query(
                ...
                KeyConditionExpression=Key("GSI1PK").eq(
                    f"TENANT#{tenant_id}#TYPE#{report_type}"
                ),
        ...
        raise DataError(f"Failed to list reports: {str(e)}", data_source="DynamoDB")
...
def delete_expired_reports():
    ...
    try:
        ...
        response = reports_table.scan(FilterExpression=Attr("expiresAt").lt(now))
        ...
        for report in expired_reports:
            try:
                ...
                reports_table.delete_item(Key={"PK": report["PK"], "SK": report["SK"]})
