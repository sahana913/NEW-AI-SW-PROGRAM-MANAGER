...
from dashboard.cache_manager import cache_report_data, get_cached_data
...
# Import cache manager from dashboard module using absolute path
...
def aggregate_report_data(
    ...
    completed_milestones = query_completed_milestones(
        tenant_id, project_ids, start_date, end_date
    )
    upcoming_milestones = query_upcoming_milestones(
        tenant_id, project_ids, days_ahead=14
    )
...
def generate_weekly_status_report_handler(
    ...
    try:
        ...
        html_content = render_html_report(
            report_data=report_data, narrative=narrative, report_type="WEEKLY_STATUS"
        )

        # Store report in S3
        s3_key = store_report_html(
            tenant_id=tenant_id, report_id=report_id, html_content=html_content
        )
...
@with_error_handling
@with_tenant_isolation
def generate_executive_summary_handler(
    event: Dict[str, Any], context: Any
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        critical_high_risks = [
            r for r in all_risks if r.get("severity") in ["CRITICAL", "HIGH"]
        ]
...
def list_reports_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info(
        "Listing reports", extra={"tenant_id": tenant_id, "report_type": report_type}
    )
    ...
    try:
        ...
        return {
            ...
            "body": json.dumps(
                {"reports": formatted_reports, "count": len(formatted_reports)}
            ),
        }

    except Exception as e:
        logger.error(
            "Failed to list reports", extra={"tenant_id": tenant_id, "error": str(e)}
        )
        raise
